...
class ComposeProject(object):
    ...
    def parse_services(self):
        for service_name, config in self.custom_modules.items():
            ...
            for k in COMPOSE_KEY_CREATE_OPTION_MAPPING:
                ret = self.create_option_to_compose_option(
                    create_option, COMPOSE_KEY_CREATE_OPTION_MAPPING[k]['parser_func'], k, COMPOSE_KEY_CREATE_OPTION_MAPPING[k]['API_key'])
                if ret != None:
                    self.Services[service_name][k] = ret
    ...
    def get_create_option_value(self, create_option, key_path):
        ...
        def dfs_create_option(create_option, key_path, level):
            if not key_path[level] in create_option:
                return None
            elif level == len(key_path) - 1:
                return create_option[key_path[level]]
            else:
                return dfs_create_option(create_option[key_path[level]], key_path, level + 1)

        if not isinstance(key_path, list):
            if not key_path in create_option:
                return None
            else:
                return create_option[key_path]
        else:
            return dfs_create_option(create_option, key_path, 0)

    def collect_create_options(self, create_option, API_keys):
        if not isinstance(API_keys, list):
            return self.get_create_option_value(create_option, API_keys)
        if not isinstance(API_keys[0], list):
            return self.get_create_option_value(create_option, API_keys)
        ...
        for key_path in API_keys:
            ...
            if ret != None:
                if not isinstance(key_path, list):
                    create_options_dict[key_path] = ret
                else:
                    create_options_dict[key_path[-1]] = ret
